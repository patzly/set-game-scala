...
import de.htwg.se.set.modelComponent.gameComponent.Player
...
class CardSpec extends AnyWordSpec with Matchers:
  ...
  "A Card" should:
    ...
    "be equal to another card with the same number, color, symbol, and shading, regardless of selection status" in:
      ...
      val card2 = Card(1, Color.RED, modelComponent.Symbol.OVAL, Shading.SOLID)
    ...
    "not be equal to another card with different number, color, symbol, or shading" in:
      val card1 = Card(1, Color.RED, modelComponent.Symbol.OVAL, Shading.SOLID)
      val card2 = Card(2, Color.GREEN, modelComponent.Symbol.SQUIGGLE, Shading.STRIPED)
    ...
    "produce a simple string representation without shading when toStringEasy is invoked" in:
      ...
      val card = Card(1, Color.RED, modelComponent.Symbol.OVAL)
